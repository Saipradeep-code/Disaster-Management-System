...
function Login() {
  ...
  const handleEmailSubmit = async (e) => {
    ...
    if (email.endsWith("@ngo.com") || email.endsWith("@ngo.org")) {
      alert("Access Denied: NGO accounts must use the designated NGO Portal to log in.");
      return;
    }

    if (!email.endsWith("@gmail.com")) {
      alert("Registration & Login is restricted to @gmail.com accounts for standard users.");
      return;
    }
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      if (!result.user.email.endsWith("@gmail.com")) {
        alert("Only @gmail.com accounts are permitted.");
        // We technically signed them in, so we should sign them out
        await auth.signOut();
        return;
      }
      localStorage.setItem("role", "user");
      ...
      <div className="w-full max-w-md z-10 animate-slide-up">
        <div className="bg-slate-900/60 backdrop-blur-xl rounded-3xl p-8 border border-slate-700/50 shadow-2xl">
          <div className="flex flex-col items-center mb-8">
            ...
            <p className="text-slate-400 text-center">Enter your credentials to access your portal</p>
          </div>
          ...
          <form onSubmit={handleEmailSubmit} className="flex flex-col gap-4">
            ...
            <button
              ...
              className={`mt-2 w-full bg-blue-600 hover:bg-blue-500 text-white font-semibold py-3.5 rounded-xl transition-all duration-300 flex items-center justify-center gap-2 group shadow-lg shadow-blue-500/25 ${isSubmitting ? 'opacity-70 cursor-not-allowed' : 'hover:shadow-blue-500/40'}`}
            >
              Sign In
